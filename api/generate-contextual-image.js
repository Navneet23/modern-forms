...
export default async function handler(req, res) {
  ...
  const { title, description, colors, styleContext, style, prompt: directPrompt } = req.body;

  // If directPrompt is provided, skip text generation and go straight to image
  // Otherwise, require styleContext for prompt generation
  if (!directPrompt && !styleContext) {
    return res.status(400).json({ error: 'Missing styleContext or prompt parameter' });
  }
    ...
    if (imageGenData.candidates && imageGenData.candidates.length > 0) {
      ...
      for (const part of parts) {
        if (part.inlineData && part.inlineData.data) {
          ...
          const filename = `contextual-image-${Date.now()}-${Math.random().toString(36).substring(7)}.${extension}`;
